...
def load_and_process_wikivoyage_data(
    url: str = config.WIKIVOYAGE_CSV_URL,
    ...
    for col in required_for_rag:
        if col in df_processed.columns:
            df_processed["content_for_rag"] += (
                f"{col.capitalize()}: " + df_processed[col] + "; "
            )

    # Clean up the RAG content
    df_processed["content_for_rag"] = (
        df_processed["content_for_rag"].str.replace(r"\s+", " ", regex=True).str.strip()
    )
